...
// A Context holds the state for drawing text from a given font
// at a given size.
type Context struct {
	r        *raster.Rasterizer
	font     *truetype.Font
...
// rp is used to actually draw the pixels.
func (c *Context) DrawText(pt raster.Point, s string, rp raster.Painter) (err os.Error) {
...
// DrawText is a more specific version of Context.DrawText.
// It draws the text onto the current image in the current color.
func (c *RGBAContext) DrawText(pt raster.Point, s string) (err os.Error) {
	c.Context.DrawText(pt, s, c.rp)
	return
}
