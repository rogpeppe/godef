...
func SpanBbox(ss []Span) draw.Rectangle {
	if len(ss) == 0 {
		return draw.ZR
	}
...
func (g *genericImagePainter) Paint(ss []Span, done bool) {
	for _, s := range ss {
		a := s.A >> 8
...
func NewPainter(dst draw.Image, src image.Image, op draw.Op) Painter {
	if src, ok := src.(image.ColorImage); ok {
		switch dst := dst.(type) {
		case *image.Alpha:
			if _, _, _, a := src.RGBA(); a == 0xffff {
				return &AlphaPainter{dst, op}
			}
