...
func walkBeforeAfter(x interface{}, before, after func(interface{})) {
	...
	// These are ordered and grouped to match /home/rog/go/src/pkg/go/ast/ast.go
	case *ast.Field:
...
func addImport(f *ast.File, ipath string, name string, renameConflicts bool) (added bool) {
	if imports(f, ipath) {
		return false
	}

	// Determine name of import.
	_, defaultName := path.Split(ipath)
	if name == "" {
		name = defaultName
	}
